...
- **数据库层控制**，比如 RLS、view、只读角色、受限 schema
- **应用层 API**，比如 `getRecentOrders(userId)`、`updateOrderStatus(orderId)`
- **手动组装上下文**，在每次模型调用前先把数据查好再传给 AI

这些做法当然能跑，但通常会在数据库、后端服务和 AI 层之间留下不少胶水代码。

ScopeDB 不是要替代 RLS 这类基础能力，而是把这套链路里“面向 AI 的访问层”收成一个可复用的抽象：边界声明一次，工具由配置生成，模型在规则内自主工作。如果你已经在用 RLS，ScopeDB 更适合放在它上面，作为 AI 的受限访问层。
